...
func Niconico(request *gorequest.SuperAgent) model.Result {
	...
	resp1, body1, errs1 := request.Get(url1).Retry(2, 5).End()
	if len(errs1) > 0 {
	...
	resp, body, errs := request.Get("https://live.nicovideo.jp/?cmnhd_ref=device=pc&site=nicolive&pos=header_servicelink&ref=WatchPage-Anchor").
	...
	if liveID != "" {
		resp, body, errs = request.Get("https://live.nicovideo.jp/watch/" + liveID).
			Timeout(10 * time.Second).
			End()
		if len(errs) > 0 {
			return model.Result{Name: name, Status: model.StatusNetworkErr, Err: errs[0]}
