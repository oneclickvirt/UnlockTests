...
func DisneyPlus(request *gorequest.SuperAgent) model.Result {
	QueryResult := requestDisney(request, "https://www.disneyplus.com", "query")
	if QueryResult.Status != model.StatusYes {
		return QueryResult
	}
	VerifyResult := requestDisney(request, "https://disney.api.edge.bamgrid.com/token", "auth")
...
// 实际进行请求
func requestDisney(request *gorequest.SuperAgent, URL string, method string) model.Result {
	name := "Disney+"
	data := url.Values{
	...
	resp, body, errs := request.Post(URL).
		...
		Set("authorization", "Bearer ZGlzbmV5JmJyb3dzZXImMS4wLjA.Cu56AgSfBTDag5NiRA81oLHkDZfu5L3CKadnefEAY84").
		Type("form").
